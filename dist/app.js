...
app.get('/api/upload/video', (_req, res) => {
    res.status(200).json({
        message: 'Yeh URL browser se GET mat karo — video bhejne ke liye POST chahiye (multipart, field name: video, Bearer token, title).',
        useMethod: 'POST',
        path: '/api/upload/video',
    });
});
app.post('/api/upload/video', requireAuth, uploadVideoMiddleware.single('video'), uploadVideoToCloudinary);
